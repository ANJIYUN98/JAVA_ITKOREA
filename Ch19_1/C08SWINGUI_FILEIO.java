...
//		panel.add(area1);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btn1) { //저장하기(out해야하므로 FileWriter)
			
			System.out.println("저장하기 CLICKED");
			
			try {
				String contents = area1.getText(); //내용을 컨텐츠로 빼옴
				String dirPath = "c:\\tmp_io\\";
				String filename = UUID.randomUUID().toString(); //날짜로 저장가능
		...
		}else if(e.getSource()==btn2) { //불러오기(in해야하므로 FileReader)
			
			System.out.println("불러오기 CLICKED");
			JFileChooser fileChooser = new JFileChooser(); //탐색기 객체 만들기
			File defaultDirPath = new File("c:\\tmp_io");  //기본경로 전달
			fileChooser.setCurrentDirectory(defaultDirPath);
			int returnVal = fileChooser.showOpenDialog(null);
			
			if(returnVal==JFileChooser.APPROVE_OPTION) //파일을 선택했다면
			{
				String filename = fileChooser.getSelectedFile().toString(); //파일탐색기에서 선택된 파일을 filename으로
